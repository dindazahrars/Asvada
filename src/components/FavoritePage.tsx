...
export default function FavoritePage() {
  ...
  // 2. Load Favorites setelah User ID ada
  useEffect(() => {
    if (currentUserId) {
      loadFavorites();
    } else if (session === null) {
      // Jika session null (logout), stop loading
      setLoading(false);
    }
  ...
  const loadFavorites = async () => {
    ...
    try {
      // Langkah A: Ambil semua ID resep dari tabel favorit user ini
      ...
      if (favoriteIds && favoriteIds.length > 0) {
        ...
        // Langkah B: Ambil detail resep berdasarkan ID yang didapat tadi
        ...
        if (recipes) {
          // Langkah C: Gabungkan data favorit dengan detail resep
          const combined = favoriteIds.map(fav => ({
            id: fav.id,
            id_recipe: fav.id_recipe,
            created_at: fav.created_at,
            resep: recipes.find(r => r.id === fav.id_recipe)
          })).filter(item => item.resep); // Hapus jika resep ternyata sudah dihapus (null)

          setFavorites(combined as any);
        }
      } else {
        setFavorites([]); // Kosongkan jika tidak ada data
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50 pt-20 pb-12">
      {/* Header Section */}
      <div className="bg-gradient-to-r from-[#FE9412] to-[#902E2B] text-white py-12 shadow-lg mb-8">
            ...
            {favorites.map((favorite) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-all cursor-pointer group border border-gray-100 relative"
                >
                  <div className="relative h-48 overflow-hidden bg-gray-200">
                    <Image
                      src={recipe.image_url || '/logo.png'} // FIX 1: Tambah fallback image
                      alt={recipe.title || 'Resep'}
                      fill
                      className="object-cover group-hover:scale-110 transition-transform duration-300"
                      unoptimized={true} // FIX 2: Agar gambar selalu muncul
                    />
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm p-2 rounded-full hover:bg-red-500 hover:text-white transition-all shadow-lg z-10 group-hover:opacity-100 opacity-100 md:opacity-0"
      ...
      {/* Stats Section */}
      {favorites.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 pb-12 mt-12">
